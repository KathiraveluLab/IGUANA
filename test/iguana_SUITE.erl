        ...
        pang ->
            ...
            case erl_epmd:port_please("test_primary_peer", "127.0.0.1") of
                {port, Port, _Version} ->
                    ...
                    TCPResult = peer:call(PeerSecondary, gen_tcp, connect, ["127.0.0.1", Port, [binary, {active, false}], 2000]),
                    io:format("Direct TCP connection result from PeerSecondary: ~p~n", [TCPResult]),
                    SSLOptions = [
                        {secure_renegotiate, true},
                        {depth, 0},
                        {verify, verify_none},
                    ...
                    SSLResult = peer:call(PeerSecondary, ssl, connect, ["127.0.0.1", Port, SSLOptions, 5000]),
                    io:format("Direct SSL connection result from PeerSecondary: ~p~n", [SSLResult]);
                Error ->
